...
function checkUniqueDefinition(actionType: string): void {
  warning(!alreadyDefined[actionType], `Duplicate definition for Action(${actionType},...`);
...
export default function duck<U>(
  ...
  return {
    ...
    reducer(state: U = initialState, action: Action = { type: 'missing action' }): U {
      const transformedState = reducers['*'] ? reducers['*'](state, action) : state;
      return reducers[action.type] ? reducers[action.type](transformedState, action) : transformedState;
    }
  };
}

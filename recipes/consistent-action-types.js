...
require('fs').readdirSync(normalizedPath).forEach((file) => require('../ducks/' + file))

describe('Consistent action type names', () => {
  Object.keys(actions).forEach((k) => {
    const a = actions[k]
    it(a, () => {
      assert.lengthOf(a.split('/'), 2, `Bad action type name ${a} (should be predixed with a duck name).`)
    })
  })
})
